...
export class BoardArticleService {
	...
	public async createBoardArticle(memberId: ObjectId, input: BoardArticleInput): Promise<BoardArticle> {
		...
		try {
			...
			console.log('Error, Service.modela:', err.message);
	...
	public async getBoardArticle(memberId: ObjectId, articleId: ObjectId): Promise<BoardArticle> {
		...
		if (memberId) {
			const viewInput = { memberId: memberId, viewRefId: articleId, viewGroup: ViewGroup.PROPERTY };
			const newView = await this.viewService.recordView(viewInput);

			if (newView) {
				await this.boardArticlesStatusEditor({ _id: articleId, targetKey: 'articleViews', modifier: 1 });
	...
	public async likeTargetBoardArticle(memberId: ObjectId, likeRefId: ObjectId): Promise<BoardArticle> {
		...
		const result = await this.boardArticlesStatusEditor({
			_id: likeRefId,
